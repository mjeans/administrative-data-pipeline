...
actual_ids <- sort(result$analytic_cohort$participant_id)
if (!setequal(actual_ids, expected_ids)) {
  stop(
    sprintf(
      "Eligible cohort mismatch. Expected: %s. Actual: %s.",
      paste(sort(expected_ids), collapse = ", "),
      paste(actual_ids, collapse = ", ")
    )
  )
}
